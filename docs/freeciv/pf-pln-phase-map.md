...
| 4 — conductance learning | Immediate execution effects and goal relief are separate; goal-neutral effects receive bounded no-progress decay, authoritative direct relief receives monotonic credit, and one pending route per category may receive idempotent downstream credit from a later same-goal change; exact failed actions/sites are suppressed before ranking; a newly grounded, same-goal-best direct city/hut completion receives the initial prior without mutating learned category state; attempt-scoped state remains atomic and truth-free | Deterministic and 2,535-decision retrospective direct-completion replay acceptance passed; four prior fresh 40-pair learning gates completed | Complete the fresh direct-completion engine pilot |
...
1. Commit the candidate-scoped direct-completion implementation and frozen
   `pressure_direct_completion_pilot_v1` design.
2. Run its 40 disjoint paired seeds and require clean source, initial-state,
   policy-isolation, safety, schema, provenance, and exact-replay acceptance.
3. Audit direct-completion floor frequency and ensure preparatory routes retain
   learned conductance.
4. Use the fresh paired score SD and effect estimate to decide whether a
   separately predeclared precision cohort is justified; do not promote or
