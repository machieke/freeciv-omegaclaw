...
1. Run the one-pair prefix of predeclared
   `pressure_defense_relevance_pilot_v1` and require the new grounded
   production-deficit context plus zero exact-replay integrity failures.
2. Run its remaining seed-disjoint pairs only if engine, schema, safety,
   source, and replay smoke gates pass.
3. Report substitutions into `city_defense`, settlement outcomes, and paired
   score before considering another semantic change.
4. Do not pool the threat-relevance estimate (+0.10, interval
   [-0.725, 1.050]) with the new cohort and do not freeze confirmatory
