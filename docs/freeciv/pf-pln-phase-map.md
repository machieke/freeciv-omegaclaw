...
| 4 — conductance learning | Grounded credit and no-progress updates are connected to live and deferred action outcomes; attempt-scoped state is atomic and feedback-ID idempotent; the completed v1 engine pilot exposed the rare-route prior bias | Implemented and live-enabled; correction required | Use an optimistic untried-route prior, then measure branch-abandonment latency in replay and a fresh paired engine cohort |
...
| 9 — full multi-goal field | Per-goal pressure, derived weights, double-count prevention, cross-goal harm, hard safety veto, budget allocation; the fresh 40-pair pressure-only v1 pilot completed with exact arm isolation and zero failures | Implemented core; v1 pilot neutral | Replace constant live goal strengths with authoritative snapshot-derived strengths, run a fresh seed-disjoint pilot, and freeze a confirmatory cohort only after a positive signal |
...
1. Correct the two v1 pilot findings: optimistic conductance for an untried
   server-advertised route, and authoritative snapshot-derived goal strengths.
2. Predeclare and execute a fresh, seed-disjoint pressure-only pilot. Do not
   pool it with v1.
