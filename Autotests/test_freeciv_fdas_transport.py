...
def _unit(unit_id, unit_type, tile, carrying=0, transported=False,
          transported_by=0):
    return {
        "activity": "idle",
        "carrying": carrying,
...
def _payload(carrying=0, transported=False, second_ferry=False,
             missing_load=False):
    ...
    units = {
        ...
        "200": _unit(200, "Trireme", 201, carrying=carrying),
    }
    if missing_load:
        units["200"].pop("carrying")
    if second_ferry:
        units["201"] = _unit(201, "Trireme", 201, carrying=0)
    payload["units"] = units
...
def test_ambiguous_carrier_omits_embark_edge_and_exact_carriage_allows_exit():
    ...
    _snapshot_value, _ruleset_value, carried = _revision(
        _payload(carrying=1, transported=True), 604)
    predicates = {
...
def test_transport_capacity_retracts_incrementally_at_full_load():
    ...
    first = _snapshot(_payload(carrying=0), 605)
    second = _snapshot(_payload(carrying=2), 606)
