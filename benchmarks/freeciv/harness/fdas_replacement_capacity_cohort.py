"""Deterministic fixed-seed audit for replacement-capacity demand."""
...
def audit_fdas_replacement_capacity_cohort(
        run_dir, expected_seeds, expected_source_commit=None, repo=None):
    """Aggregate all fixed games without selecting on demand frequency."""
    ...
    if not expected_seeds or len(expected_seeds) != len(set(expected_seeds)):
        raise ValueError("replacement capacity cohort requires unique seeds")
    games_root = os.path.join(run_dir, "games", "main", "e_full_loop")
    rows = []
    for seed in expected_seeds:
        game_dir = os.path.join(games_root, "{}-00".format(seed))
        report = audit_fdas_replacement_capacity_live(game_dir, repo=repo)
        rows.append({
            "accepted": report["acceptance"]["accepted"],
            "capacity_deficit_goals": report["summary"][
                "capacity_deficit_goals"],
    ...
    checks = {
        ...
        "all_game_level_capacity_audits_pass": all(
        ...
        "all_sources_are_clean_and_commit_bound": all(
            ...
            for row in rows),
        "every_game_contributes_shadow_evaluations": all(
            row["evaluations"] > 0 for row in rows),
    }
    report = {
        "acceptance": {"accepted": all(checks.values()), "checks": checks},
        "claim_scope": (
            "fixed fresh-game distribution of typed source-city replacement-"
            "capacity demand and legal shadow production candidates only; no "
            "completion, value, action, score, or win-rate claim"),
        "evidence": {
            "run-summary.json": {
        ...
        "schema_version": "1.0",
        "summary": {
            "capacity_deficit_goal_count": sum(
                row["capacity_deficit_goals"] for row in rows),
            ...
            "game_count": len(rows),
            "games_with_capacity_demand": sum(
                row["capacity_deficit_goals"] > 0 for row in rows),
            "games_with_capacity_production_candidate": sum(
                row["capacity_production_candidates"] > 0 for row in rows),
        },
    }
