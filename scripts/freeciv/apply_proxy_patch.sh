...
for patch_spec in \
  ...
  "$strategic_observability_patch:$pinned_strategic_observability_sha256"; do
...
if git -C "$upstream_root" apply --reverse --check "$strategic_observability_patch" >/dev/null 2>&1; then
  echo "PLN authoritative-state through strategic-observability patches are already applied"
  exit 0
fi
...
for patch_file in \
  ...
  "$strategic_observability_patch"; do
