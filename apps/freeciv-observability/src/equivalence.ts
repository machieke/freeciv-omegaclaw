...
export const replayProjection = (state: ReplayState): unknown => orderedObject({
  ...
  transitionValueEstimates: state.transitionValueEstimates.map((event) => event.event_id),
  transitionValueUpdates: state.transitionValueUpdates.map((event) => event.event_id),
  pathPersistenceEvents: state.pathPersistenceEvents.map((event) => event.event_id),
